-- ONEDECORE -- the premium public requirement form, as contract version v3.
--
-- WHY A NEW VERSION AGAIN
--
-- The owner-approved public form now asks two questions v2 explicitly forbids:
-- the scope of the project (kitchen / 1-2-3 BHK / villa) and a budget range
-- drawn from that scope's own ladder.
--
-- v2 forbids them because ITS form never showed them, and that prohibition is
-- what makes a v2 row readable: a null budget under v2 means "never asked", not
-- "asked and lost". Relaxing v2 would delete that distinction from every row
-- already stored under it. So v2 is untouched, down to its `budget_not_asked`
-- exception, and v3 is added beside it -- the same reasoning that added v2
-- beside v1.
--
-- SCOPE IS NOT SERVICE
--
-- "2 BHK" is the size of the home, not a service ONEDECORE sells.
-- `leads.service_code` therefore keeps its three-value allowlist, the scope goes
-- in its own column, and the function CHECKS that the submitted service is the
-- one the scope implies rather than believing a client that asserts both.
--
-- WHAT IS ENFORCED HERE AND WHY IT IS DUPLICATED
--
-- The application validates the same three pairings. That duplication is the
-- point: this function is SECURITY DEFINER and is the only way a row reaches
-- `public.leads`, so it must hold on its own against a caller that never ran
-- the application's validator.
--
-- HOW THIS FILE WAS PRODUCED
--
-- Generated from `20260907130000_public_consultation_single_step_v2.sql` by
-- script, with edits confined to: the signature (two new defaulted arguments),
-- the planner allowlist, one guard forbidding the new fields outside v3, the v3
-- branch itself, and two lines of the `leads` insert. Everything else is
-- byte-identical to what is deployed.
--
-- CREATE OR REPLACE cannot change a signature, so the two new arguments are
-- DEFAULTED -- which makes this an overload, not a replacement. The old
-- 29-argument function is dropped explicitly below AFTER the new one exists, so
-- no window passes with no function present and no caller resolves to the stale
-- overload. Privileges are re-asserted regardless, because a definer function
-- that falls back to the PUBLIC default becomes callable by anon.

-- 1. Columns. Nullable with no default and no backfill: existing rows were
--    collected by forms that never asked, and null is how that is recorded.
alter table public.leads
  add column if not exists project_scope_code text,
  add column if not exists budget_range_code text;

comment on column public.leads.project_scope_code is
  'Project scope from the public requirement form (public-consult-v3). Null when the form did not ask. Not a service code.';
comment on column public.leads.budget_range_code is
  'Budget band from the project scope ladder (public-consult-v3). Null when the form did not ask.';

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'leads_project_scope_code_check'
  ) then
    alter table public.leads add constraint leads_project_scope_code_check
      check (project_scope_code is null or project_scope_code in
        ('kitchen', '1-bhk', '2-bhk', '3-bhk', 'villa'));
  end if;

  if not exists (
    select 1 from pg_constraint where conname = 'leads_budget_range_code_check'
  ) then
    alter table public.leads add constraint leads_budget_range_code_check
      check (budget_range_code is null or budget_range_code in (
        'kitchen-below-1l', 'kitchen-1-2l', 'kitchen-2-3l', 'kitchen-above-3l',
        '1bhk-3-5l', '1bhk-5-7l', '1bhk-7-10l', '1bhk-above-10l',
        '2bhk-4-8l', '2bhk-8-12l', '2bhk-12-16l', '2bhk-above-16l',
        '3bhk-5-9l', '3bhk-9-13l', '3bhk-13-16l', '3bhk-above-16l',
        'villa-5-10l', 'villa-10-15l', 'villa-15-20l', 'villa-above-20l'));
  end if;

  -- A budget without a scope cannot be read back: the ladders share labels, so
  -- the scope is what gives the code its meaning.
  if not exists (
    select 1 from pg_constraint where conname = 'leads_budget_requires_scope_check'
  ) then
    alter table public.leads add constraint leads_budget_requires_scope_check
      check (budget_range_code is null or project_scope_code is not null);
  end if;
end
$$;
...
begin
  ...
  if p_planner_version <> 'public-consult-v3'
...
-- 3. Retire the 29-argument overload.
--
-- Dropped only now, after the 31-argument version exists, so the endpoint is
-- never without a function and no caller silently resolves to the old shape.
