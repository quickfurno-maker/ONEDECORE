-- =============================================================================
-- ONEDECORE Phase 5F-B — Controlled Public Lead Activation Hardening (M17)
-- Local implementation only. No public activation. No managed apply in 5F-B.
-- =============================================================================

create or replace function private.resolve_lead_intake_contact_by_phone(p_phone_e164 text)
returns uuid
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  select count(distinct ch.contact_id)::integer
  into v_distinct_count
  from public.contact_channels ch
  inner join public.contacts c on c.id = ch.contact_id
  where ch.channel_type = 'phone'
    and ch.address_normalized = p_phone_e164
    and ch.status in ('active', 'suppressed')
    and c.status not in ('merged', 'archived');

  if v_distinct_count > 1 then
    raise exception 'contact_identity:ambiguous phone' using errcode = 'P0001';
  end if;
  ...
  where ch.channel_type = 'phone'
    ...
    and c.status not in ('merged', 'archived')
  order by
    case ch.status when 'active' then 0 else 1 end,
    ch.updated_at desc,
    ch.id asc
  limit 1;
...
comment on function private.resolve_lead_intake_contact_by_phone(text) is
  'Phase 5F-B intake identity resolver. Finds contact by normalized phone across active/suppressed channels. Fails on ambiguous identities.';

alter function private.resolve_lead_intake_contact_by_phone(text) owner to postgres;

revoke all on function private.resolve_lead_intake_contact_by_phone(text) from public, anon, authenticated;
