...
select results_eq(
  ...
  array[108],
  'Should have exactly 108 seeded system permissions (6 foundation + 2 portfolio + 4 lead intake + 9 Phase 5B CRM + 2 Phase 5C2B manual lead + 3 Phase 5D bulk import + 3 Phase 5E targets/reporting + 3 Phase 6B inbox + 12 Phase 6D staff attendance leave + 3 Phase 7A commercial quotations + 2 Phase 7B finalization send + 3 Phase 8A projects + 6 Phase 8B project design + 5 Phase 8C project execution + 5 Phase 9A campaigns/consent + 5 Phase 9B landing lab + 3 Phase 9C-B execution + 6 Phase 9D-B commerce + 1 CRM 2A-2 sla.manage + 1 CRM 2C cadences.manage + 1 Workforce V1 attendance.approve + 2 Workforce V1 salary.manage/salary.self + 2 Sales Manager control plane leads.delete/projects.read_high_level + 1 Website Manager website.manage + 3 WM-2 whatsapp.templates.read/use/manage + 6 WM-3 whatsapp.contacts.read/opt_out.record/segments.read/segments.manage/settings.read/settings.manage + 3 WM-4 whatsapp.campaigns.execute/test_send/cancel + 2 WM-5 whatsapp.analytics.read/reports.export + 4 WM-6 whatsapp.automations.read/manage + whatsapp.flows.read/manage) with is_system = true'
);
...
select results_eq(
  ...
  array[154],
  'Public schema must contain exactly the 154 authorized application tables (… + Phase 9D-D1 COD orders + CRM 2A-1 lead_activity_outcome_codes + lead_follow_up_events + CRM 2A-2 crm_sla_policies + crm_sla_clocks + CRM 2C crm_cadence_templates + crm_cadence_steps + crm_lead_cadence_enrollments + crm_cadence_enrollment_events + Workforce V1 attendance_submissions + attendance_submission_events + salary_profiles + salary_statements + salary_statement_lines + salary_payments + salary_statement_events + portfolio_project_categories + Website Manager website_homepage_versions + website_homepage_sections + website_homepage_banners + website_homepage_publication + WM-1 whatsapp_conversation_staff_state + WM-2 whatsapp_template_snapshots + whatsapp_template_sync_runs + whatsapp_template_submissions + whatsapp_template_status_events + whatsapp_media_access_events + whatsapp_template_send_intents + whatsapp_template_dispatch_attempts + WM-3 whatsapp_marketing_preference_events + whatsapp_marketing_send_policies + whatsapp_segments + WM-4 whatsapp_campaign_specs + whatsapp_campaign_runs + whatsapp_campaign_recipients + whatsapp_campaign_dispatch_jobs + whatsapp_campaign_dispatch_events + whatsapp_message_campaign_attributions + whatsapp_campaign_test_sends + WM-5 whatsapp_click_destinations + whatsapp_click_tokens + whatsapp_click_events + whatsapp_reply_attributions + whatsapp_report_exports + WM-6 whatsapp_automations + whatsapp_automation_enrollments + whatsapp_automation_events + whatsapp_message_automation_attributions + whatsapp_flows + whatsapp_flow_provider_requests + whatsapp_flow_provider_events + whatsapp_flow_tokens + whatsapp_flow_responses + whatsapp_referral_contexts)'
);
