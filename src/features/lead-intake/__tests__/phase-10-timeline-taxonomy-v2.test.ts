...
describe("PR94 timeline taxonomy v2", () => {
  ...
  test("the timeline vocabulary is rendered from PM_PLANNER, once", () => {
    /*
     * The timeline is asked in the one-panel form now, so the brief is where
     * the options are rendered. What must stay true is unchanged: no surface
     * hardcodes a code, and the retired v1 codes appear nowhere.
     */
    ...
    assert.match(capture, /PM_PLANNER\.timelines/);
