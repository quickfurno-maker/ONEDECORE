...
function buildCompleteFormDefaults(
  ...
  const resolutionOptions = getCompletionResolutionOptions({
    leadStatus,
  ...
  return {
    ...
    resolution: getDefaultCompletionResolution(resolutionOptions),
    nextType: suggested,
...
function CompleteActivityForm({
  ...
  const showResolutionChooser = resolutionOptions.length > 1;

  const submitForm = (formData: FormData) => {
    setClientError(null);
    const mappings = [
      {
        local: "onHoldReviewAtLocal",
        absolute: "onHoldReviewAt",
        required: false,
      },
      { local: "nextDueAtLocal", absolute: "nextDueAt", required: false },
      {
        local: "nextReminderAtLocal",
        absolute: "nextReminderAt",
        required: false,
      },
    ];
