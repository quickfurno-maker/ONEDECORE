...
export const PROCESSOR_REGISTER: readonly ProcessorRegisterEntry[] = [
  ...
  {
    provider: "Meta Platforms — advertising measurement",
    status: "current",
    purpose: "Consent-dependent Meta advertising measurement and campaign attribution",
    dataCategories: [
      "Event name, time and deduplication identifier",
      "Source page URL and action source",
      "Browser user agent and trusted client IP",
      "_fbp and _fbc advertising cookies when present",
    ],
    locationsKnown: null,
    transferAssessment: "Meta provider locations and sub-processors are governed by current provider terms; no India-only processing claim is made.",
    contractDpa: "Provider terms apply; bespoke signed agreement not claimed.",
    securityReview: "CONSENT-GATED — automatic advanced matching off; server token remains server-only.",
    deletionCapability: "Withdrawal stops future ONEDECORE submissions and clears browser-accessible _fbp/_fbc; provider-side retention follows Meta terms.",
    incidentContact: null,
    approval: null,
    notes: [
      "ONEDECORE first-party website analytics is stored in the existing Supabase processor; no additional analytics processor is introduced.",
      "CONSENT-DEPENDENT. First-party analytics and Meta measurement run only after the visitor chooses Allow analytics & advertising cookies (onedecore_ad_tracking_consent v2).",
      "ONEDECORE analytics tables contain anonymous visitor/session UUIDs, traffic-source metadata and bounded site events; customer PII is not copied into those analytics tables.",
      "Never sent to Meta: name, phone, email, postal address, date of birth, gender, budget, service selection, project scope, enquiry text, quotation data or CRM identifiers. Automatic advanced matching is OFF.",
      "Disclosure: Privacy Notice v1.2, section 'Cookies, analytics and advertising measurement'.",
    ],
  },
] as const;
