...
- **Public Portfolio Experience Architecture Freeze (Phase 2E3A):** Public route map (`/portfolio`, `/portfolio/[slug]`, homepage integration `/`), DTO data contract, `onedecore.in` canonical site identity (`src/config/site.ts`), bounded pagination (12 cards/page, 13-row request), Outcome A RLS verification (no schema migration required), Sharp WebP image delivery model, SEO/OpenGraph/Sitemap architecture, tag-based cache revalidation strategy (`unstable_cache`, 2-arg `revalidateTag`), WCAG 2.1 AA accessibility freeze, anti-N+1 query budget ($\le 3$ queries/route), security threat matrix, and Phase 2E3B file plan (`ADR-0016`, `ADR-0017`, `phase-2e3a-public-portfolio-architecture.md`).
