...
- **Database Schema & Hardening:** Identity & RBAC foundation (`profiles`, `roles`, `permissions`, `role_permissions`, `user_roles`) with strict column-level grants, 100% RLS coverage, active staff profile status requirement (`status = 'active'`), `public.authorize(text)` RPC, and pgTAP test suite (44 subtests).
