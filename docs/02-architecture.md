...
**Document Status:** Locked Architecture Baseline (truth-synced post Phase 9A M31 repository implementation, August 17, 2026)
**Pattern:** Modular Monolith
**Framework:** Next.js 16.2.11
**Database:** Supabase PostgreSQL
**Route Prefix:** Internal CRM uses `/admin`
**Current Phase:** Phase 9A architecture COMPLETE_FROZEN; repository implementation REPOSITORY_COMPLETE (M31 not managed-applied). Phase 8C COMPLETE on main (PR #61 merged). Architecture PR #62 merged `caff9d0864e1546dff38646df4355dafa851a473`.
...
| Campaigns (Phase 9A repository) | Draft → pending_approval → approved/rejected; MARKETING via existing `consent_events`; DNC + channel suppression reused; freeze audience **rules** not recipient PII; no 9B FK; no 9C execution; M31 repository-only |

Phase 6A delivers managed WhatsApp **data/webhook foundation** (migration 18). Phase 6B delivers managed **inbox/send-intent/dispatch foundations** (migrations 19–21) plus repository admin UI — **not production-activated**. Phase 8C project execution is **COMPLETE** (M30 managed). Phase 9A architecture is **COMPLETE_FROZEN** (ADR-0027 / PR #62). Phase 9A M31 is **repository-complete, not managed-applied**; `/admin/campaigns` is staff governance UI only.
