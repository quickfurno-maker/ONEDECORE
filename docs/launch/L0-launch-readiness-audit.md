...
**Live deployed SHA:** **UNKNOWN — owner action required** (see BLOCK-01)
...
| --- | --- | --- | --- | --- |
| **BLOCK-01** | **Live deployed SHA is unknown.** `/api/health` returns no build identifier and no deploy record exists in the repo. We cannot state what code is serving onedecore.in. | OWNER+ENG | OPEN | `git rev-parse HEAD` from the VPS deploy directory, pasted into this register. Then (ENG) add a build SHA to `/api/health` so this is never a manual question again. |
...
| Production main SHA and live VPS SHA recorded | **NOT MET** — BLOCK-01, owner action |

**L0 is complete except for BLOCK-01**, which needs one command run on the VPS.
...
1. **BLOCK-01** — one command, unblocks everything else being verifiable.
2. **BLOCK-03 + BLOCK-04** (L2) — the legal text currently *denies* that
   ...
   it.
3. **BLOCK-05, BLOCK-06, BLOCK-07** — the attribution defects. These are the
   ...
   L3, not after it.
4. **BLOCK-02 + BLOCK-11** (L3/L4/L6) — the measurement layer itself.
5. **BLOCK-08, BLOCK-09, BLOCK-10, BLOCK-15** (L1/L5) — the WhatsApp lane and
   the public phone decision. Journey D cannot be certified without them, and
   the Click-to-WhatsApp campaign cannot exist.
6. **BLOCK-12, BLOCK-13, BLOCK-14** — verification and account ownership.
