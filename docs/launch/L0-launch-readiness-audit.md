# L0 â€” Public Launch Readiness Audit and Blocker Register
...
**Live deployed SHA:** **UNKNOWN â€” owner action required** (see BLOCK-01)
...
- **CODE** â€” read from the repository at the audited head.
- **LIVE** â€” observed from `https://onedecore.in` by unauthenticated read-only
  HTTP GET on the audit date. No form was submitted and no lead was created.

---

## 1. Launch surface â€” what is actually public
...
| `/interiors` | Interiors conversion page | Yes â€” primary |
...
| `/shop`, `/shop/*` | COD storefront (7 routes) | Out of launch scope â€” see Â§6 |
...
| `GET /api/health` | Process probe. Returns `{ok, service}` only â€” **no SHA**. |
...
- Homepage responds with `Cache-Control: s-maxage=31536000` (one year shared
  cache). See RISK-02 â€” this interacts badly with adding tags.
...
| Separated consent (service enquiry / phone / WhatsApp) | **Built** | CODE: `ConsultationLeadForm.tsx` â€” `serviceEnquiryConsent`, `servicePhoneConsent`, `whatsappService`. **No marketing consent is fabricated.** |
...
| Lead-intake legal/consent activation gate | **GREEN** | CODE: executed â€” activation complete, processors ready, consent versions effective, legal mode `published` |
...
### 3.1 Launch blockers â€” no paid spend until every one is DONE
...
| **BLOCK-05** | **Attribution does not survive navigation.** `collectLeadFormAttribution()` reads only `window.location.search` at submit time. A visitor who lands on `/interiors?gclid=â€¦` and submits from `/` loses the click ID entirely. There is no cookie, `sessionStorage` or first-touch persistence anywhere. Real ad traffic browses before converting, so a large share of paid leads will arrive with **no attribution**. | ENG | OPEN | First-touch attribution persisted across a same-session multi-page journey; test lead landing on `/interiors?utm_*&gclid=â€¦`, navigating to `/portfolio` then `/`, submitting, and arriving in CRM with the original values intact. |
...
| **BLOCK-12** | **Lead-intake production mode unverified.** `ONEDECORE_LEAD_INTAKE_MODE` and `ONEDECORE_TRUST_PROXY` cannot be read remotely. All **code** gates are GREEN (activation complete, processors ready, consent versions effective, legal published), so only the environment is in question â€” but no lead has been proven to reach CRM from production. | OWNER+ENG | VERIFY | One real submission on production reaching CRM with correct source, consent rows and no duplicate. |
...
| **BLOCK-14** | **Google assets not verified.** Search Console verification, sitemap submission, Google Business Profile, Google Ads account and billing, GA4 â†” Google Ads link. | OWNER | OPEN | Search Console verified with sitemap accepted; GBP verified; Google Ads billing active; account link confirmed. |
...
### 3.2 Should-fix before spend â€” not hard blockers
...
| RISK-07 | Public contact identity is `onedecore@gmail.com` â€” a consumer mailbox on a business with a verified domain. | OWNER | OWNER-INPUT | Domain mailbox, or explicit owner decision to keep it. |
...
| `NEXT_PUBLIC_ONEDECORE_LEAD_FORM_MODE` | Form UI: `copy-only` / `preview` / `active` | `copy-only` â€” **LIVE: `active`** |
| `ONEDECORE_WHATSAPP_WEBHOOK_MODE` | Meta inbound webhook | `disabled` |
| `ONEDECORE_WHATSAPP_OUTBOUND_MODE` | Provider dispatch | `disabled` |
| `ONEDECORE_SHOP_PUBLIC_ENABLED` | Public storefront | `false` â€” **LIVE: enabled** |
...
In-code activation gates (not environment â€” these are owner facts recorded in
...
| Need | State |
| --- | --- |
| Lead attribution storage | **Ready** â€” `leads.attribution` (jsonb) |
| UTM + `fbclid`/`gclid`/`wbraid`/`gbraid`/`fbc`/`fbp` normalisation | **Ready** â€” allow-listed key projection, migration `20260821140000` |
| First-touchpoint projection | **Ready** â€” trigger-driven, with campaign-reference validation |
| Campaign attribution / metrics / conversion-feedback tables | **Ready** â€” migration `20260821140000` |
| Consent records, versions, suppression | **Ready** â€” consent registry effective |
| WhatsApp identity, conversation, message, status | **Ready** â€” Phase 6B foundations |
| Conversion feedback **delivery** to providers | **Missing** â€” see SEQ-01. Tables exist; nothing sends. |

---

## 6. Explicitly deferred (L12) â€” must not delay launch
...
7. Additional commerce and admin refinements â€” **except** the RISK-08 decision
...
| --- | --- |
| Exact public launch surface is known | **MET** â€” Â§1, code + live |
| Every remaining task classified blocker / post-launch / deferred | **MET** â€” Â§3.1, Â§3.2, Â§3.3, Â§6 |
| No ambiguous "maybe needed" items remain | **MET** â€” every row has an owner and required evidence |
| No production behaviour change in this phase | **MET** â€” audit only; read-only HTTP GET; no form submitted, no lead created |
| Production main SHA and live VPS SHA recorded | **NOT MET** â€” BLOCK-01, owner action |
...
1. **BLOCK-01** â€” one command, unblocks everything else being verifiable.
2. **BLOCK-03 + BLOCK-04** (L2) â€” the legal text currently *denies* that
   ...
   it.
3. **BLOCK-05, BLOCK-06, BLOCK-07** â€” the attribution defects. These are the
   ...
   L3, not after it.
4. **BLOCK-02 + BLOCK-11** (L3/L4/L6) â€” the measurement layer itself.
5. **BLOCK-08, BLOCK-09, BLOCK-10, BLOCK-15** (L1/L5) â€” the WhatsApp lane and
   the public phone decision. Journey D cannot be certified without them, and
   the Click-to-WhatsApp campaign cannot exist.
6. **BLOCK-12, BLOCK-13, BLOCK-14** â€” verification and account ownership.
...
funnel, not a rebuild â€” with the exception that the WhatsApp CTA must be added
